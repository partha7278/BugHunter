...
class ActionController extends Controller
{
    ...
    public function catsprogress(Request $request){
        ...
        $cats = DB::select(DB::raw( "select  derived.completed as comp, cntbug.tbug as tobug from b_cat
                inner join ( select bug.b_cat_id as bug_cat, COUNT(bug.id) as completed from bug, testing where bug.id=testing.bug_id and testing.url_id=$urlid and testing.complete='true' and bug.b_cat_id=$catid  GROUP BY bug.b_cat_id
                          ) derived on b_cat.id = derived.bug_cat
                inner join ( select bug.b_cat_id as bcat ,COUNT(bug.id)  as tbug from bug where bug.b_cat_id=$catid GROUP BY bug.b_cat_id
