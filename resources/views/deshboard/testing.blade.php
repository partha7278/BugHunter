...
@section('content')
    ...
    <script>
        ...
        $.ajax({
            ...
            type: 'post',
            data:{ catid:catid , urlid:url  ,_token:token },
            success:function(cats){
                if(cats[0]){
                var st = (cats[0].comp / cats[0].tobug * 100).toFixed(0);
                $('#menu-header-'+catid).css("background-image", "linear-gradient(to right,#476b6b "+st+"%,#55446D 0% )");
                }
            }
        });
    }
    ...
    <div class="main-content">
        ...
        <div class="test-bug-cat-menu">
            ...
            @foreach($category as $cat)
                ...
                <script> catsprogress( {{ $cat->id }} ); </script>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="col-md-4"  style="display:inline-flex;">
                                                ...
                                                <div class="test-item-list-com">
                                                    <label class="switch">
        									             <input type="checkbox" @foreach($url->testings as $testing) @if($testing->bug_id == $bug->id) @if($testing->complete == 'true') checked @endif @endif @endforeach  name="complete" id="com-{{ $bug->id }}" onclick="complete( {{ $bug->id }},{{ $bug->b_cat_id }} )"><span class="slider round"></span>
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
    ...
    <script>
        ...
        function complete(bug_id, catid){
